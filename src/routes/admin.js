...
router.get('/stats', async (req, res) => {
  try {
    const [views, clicks, topLinks, dailyViews, dailyClicks] = await Promise.all([
...
router.get('/visitors', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT id, ip, user_agent, referer, visited_at FROM page_views ORDER BY visited_at DESC LIMIT $1',
      [limit]
    );
